...
import { BashPlain, JavascriptPlain, JavaPlain, CsharpPlain, PythonPlain, PhpPlain, GoPlain, RubyPlain } from 'devicons-react';
...
Note, a Buildstash [application-level API key](/builds-and-releases/connect-ci-cd#generate-an-application-api-token) is required for uploading builds.
...
<Steps>
  <Step title="Request to start upload" icon="chevron-right" stepNumber={1}>
    Use [/upload/request](/api/endpoint/upload/request) endpoint to pass in all key build information, request to begin the upload process, and receive a presigned URL for uploading the build file to.

    <Cards>
      <Card title="/upload/request" icon={<SquareTerminal />} horizontal href="/api/endpoint/upload/request">
      </Card>
    </Cards>
  </Step>
</Steps>
...
<Steps>
  <Step title="Upload file to presigned URL" icon="chevron-right" stepNumber={2}>
    ...
    ```bash tab="<BashPlain color='currentColor' />cURL"
    ...
    ```javascript tab="<JavascriptPlain color='currentColor' />JavaScript"
    ...
    ```python tab="<PythonPlain color='currentColor' />Python"
    # uploadRequest made as in above step
    ...
    ```php tab="<PhpPlain color='currentColor' />PHP"
    // uploadRequest made as in above step
    $primary_file = $uploadRequest["data"]["primary_file"];
    $primaryFilePath = "path/to/your/file.exe";
    ...
    ```go tab="<GoPlain color='currentColor' />Go"
    // uploadRequest made as in above step
    ...
    ```java tab="<JavaPlain color='currentColor' />Java"
    // uploadRequest made as in above step
    ...
    ```csharp tab="<CsharpPlain color='currentColor' />C#"
    // uploadRequest made as in above step
    ...
    ```ruby tab="<RubyPlain color='currentColor' />Ruby"
    # uploadRequest made as in above step
...
Alternatively, if your primary build file is larger than 5GB, you must upload as multiple parts. Your original upload request will also return a chunked_upload boolean indicating whether multipart upload is advised and available for this build.
  ...
  <Steps>
    <Step title="Request to start upload" icon="chevron-right" stepNumber={1}>
      Use [/upload/request/multipart](/api/endpoint/upload/request-multipart) endpoint to request the presigned URL for each part.

    <Cards>
      <Card title="/upload/request/multipart" icon={<SquareTerminal />} horizontal href="/api/endpoint/upload/request-multipart">
      </Card>
    </Cards>
    </Step>
  ...
  In addition to your primary build file, you may also optionally upload an expansion file, where this is supported by the target platform on Buildstash. An example of this is uploading an OBB expansion file to accompany a primary APK.
    ...
    <Steps>
      <Step title="Include expansion file details in initial start upload request" icon="chevron-right" stepNumber={1}>
        When you call [/upload/request](/api/endpoint/upload/request) endpoint in the first step, you'll include the expansion file details there, and receive the presigned URL to upload to, similarly to with the primary file.
      </Step>
...
---
  ...
  <Cards>
      <Card title="CI Integrations" href="/integrations" icon={<ToyBrickIcon />}>
